...
const LeanSolutionCard = memo(({ solution }: Props) => {
  return (
    ...
    >
      ...
      <h4 className="text-base font-bold text-white mb-1.5 group-hover:text-[#F4845F] transition-colors line-clamp-1">
        {solution.title}
      </h4>
      <p className="text-sm text-white/70 leading-snug mb-2 line-clamp-2">
        {solution.tagline}
      </p>
      {solution.description && (
        <p className="text-sm text-white/60 leading-snug mb-4 line-clamp-2">
          {solution.description}
        </p>
      )}
