...
const HowWeImplement = memo(({ hideHeader = false, compact = false }: HowWeImplementProps) => {
  ...
  const costBadge = (
    <div className="flex items-center justify-center gap-2 rounded-full border border-[#F4845F]/40 bg-[#0B1224] px-3 py-1.5 shadow-lg shadow-black/20 w-full">
      <span className="h-1.5 w-1.5 rounded-full bg-[#F4845F] shrink-0" />
      <span className={`font-semibold text-[#F4845F] ${compact ? 'text-[11px]' : 'text-sm'}`}>
  ...
  return (
    <section className={`bg-[#0B1224] ${compact ? 'pt-2 pb-10' : 'py-16 md:py-20 border-t border-white/5'}`}>
      <div className={`container mx-auto px-4 sm:px-6 lg:px-8 ${compact ? 'max-w-5xl' : 'max-w-6xl'}`}>
        ...
        <div className="relative">
          <ol className={`grid grid-cols-1 md:grid-cols-5 ${compact ? 'gap-3' : 'gap-4'}`}>
            {howWeImplement.steps.map((step, idx) => (
              <Fragment key={step.n}>
                <li
                  className={`relative rounded-xl bg-white/5 border border-white/10 hover:border-[#F4845F]/50 transition-all duration-500 ${compact ? 'p-3' : 'p-5'}`}
                >
                  <span className={`font-bold text-[#F4845F]/80 leading-none block ${compact ? 'text-2xl mb-1.5' : 'text-3xl mb-3'}`}>
                    {step.n}
                  </span>
                  <h3 className={`font-bold text-white mb-1 ${compact ? 'text-xs' : 'text-sm'}`}>{step.title}</h3>
                  <p className={`text-white/60 ${compact ? 'text-[11px] leading-relaxed' : 'text-xs leading-relaxed'}`}>{step.description}</p>
                </li>
                {idx === 2 && (
