...
const ResultCard = ({ icon, title, description, index, backgroundColor, backgroundOpacity }: ResultCardProps) => {
  return (
    <div
      className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 aspect-square flex flex-col group"
    ...
    >
      ...
      <div className="flex-grow flex items-end">
        <p className="text-gray-600 text-center text-xs leading-relaxed w-full">
          {description}
        </p>
      </div>
    </div>
