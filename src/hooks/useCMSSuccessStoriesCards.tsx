...
export const useCMSSuccessStoriesCards = () => {
  ...
  const fetchCards = useCallback(async (pageSlugOrId: string, language: string = 'en') => {
    if (!pageSlugOrId) {
      console.log('useCMSSuccessStoriesCards - No pageSlugOrId provided');
      setCards([]);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('useCMSSuccessStoriesCards - Query result:', { data, error });
  ...
  return {
    ...
    loading,
    fetchCards,
    createCard: useCallback(async (pageSlugOrId: string, language: string, cardData: Omit<SuccessStoryCard, 'id' | 'page_id' | 'language' | 'created_at' | 'updated_at'>) => {
      try {
        // Convert slug to page ID if needed
        let pageId = pageSlugOrId;
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(pageSlugOrId);
    ...
    updateCard: useCallback(async (cardId: string, updates: Partial<SuccessStoryCard>) => {
      try {
        const { data, error } = await supabase
          .from('cms_success_stories_cards')
          .update(updates)
    ...
    updateIsActiveHome: useCallback(async (cardId: string, isActiveHome: boolean) => {
      try {
        const { data, error } = await supabase
          .from('cms_success_stories_cards')
          .update({ is_active_home: isActiveHome })
    ...
    deleteCard: useCallback(async (cardId: string) => {
      try {
        const { error } = await supabase
          .from('cms_success_stories_cards')
          .update({ is_active: false })
    ...
    reorderCards: useCallback(async (pageSlugOrId: string, language: string, cardIds: string[]) => {
      try {
        const updates = cardIds.map((cardId, index) => ({
          id: cardId,
          card_order: index + 1,
