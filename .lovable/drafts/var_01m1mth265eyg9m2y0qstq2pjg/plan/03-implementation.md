...
- `src/components/comunidade/CommunityOpening.tsx`
  - o bloco decorativo `hidden md:block` passa a mapear as cinco imagens com posições absolutas fixas (right/top/bottom variados), tamanhos distintos, `rotate-[…]` entre -6° e +7°, `opacity` entre 55% e 80%, `ring-1 ring-border` e `rounded-[1.25rem–2rem]`;
