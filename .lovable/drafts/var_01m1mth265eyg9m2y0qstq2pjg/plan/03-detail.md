...
4. **Rotas claras no header.** `/success-stories` **não** está na lista (hoje: `/`, `/i6-builders`, `/community`, `/contact`, `/i6-blog`, `/insights`, `/i6-intelligence`, `/our-ai`, `/docs` e subpáginas). Precisa ser adicionada, junto com `/success-stories/<slug>` — mas atenção: a capa do detalhe é escura no topo, então para o detalhe a regra de header claro não deve ser aplicada, só para a listagem.
